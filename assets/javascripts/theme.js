										...
										'<div class="row">',
											'<div class="col-xs-6">',
												'<p class="mt-xs mb-none">',
													'<a href="../../sys/logout.php">Trocar Usuário</a>',
												'</p>',
											'</div>',
											'<div class="col-xs-6 text-right">',
												'<button type="submit" class="btn btn-primary">Destravar</button>',
											'</div>',
										'</div>',
									'</form>',
								'</div>',
...
(function( $ ) {
	...
	$(function() {
		$('.panel')
			.on( 'click', '.panel-action a.fa-caret-up', function( e ) {
				e.preventDefault();
			...
			.on( 'click', '.panel-action a.fa-caret-down', function( e ) {
				e.preventDefault();
			...
			.on( 'click', '.panel-action a.fa-times', function( e ) {
				e.preventDefault();
...
(function(theme, $) {
	...
	PluginPortlet.prototype = {
		...
		saveState: function( panel ) {
			...
			var collapse = panel.find('.panel-action').children('a.fa-caret-up, a.fa-caret-down'),
			...
			if (!!data) {
				$.each(data, function(panelId, state) {
					...
					if (!panel.data('portlet-state-loaded')) {
						if (state == 'collapsed') {
							panel.find('.panel-action a.fa-caret-down').trigger('click');
						} else if (state == 'removed') {
							panel.find('.panel-action a.fa-times').trigger('click');
						}
		...
		events: function($el) {
			...
			$el.find('.panel-action').on( 'click', 'a.fa-caret-up, a.fa-caret-down, a.fa-times', function( e ) {
				setTimeout(function() {
					_self.saveState( $el );
				}, 250);
			});
	...
	if ( $.isFunction( $.fn[ 'dataTable' ] ) ) {
		...
		$.extend(true, $.fn.dataTable.defaults, {
			...
			oLanguage: {
				...
				sInfoEmpty: 'Sem resultados disponíveis',
			...
			fnInitComplete: function( settings, json ) {
				...
				$search
					.attr({
						placeholder: typeof options.searchPlaceholder !== 'undefined' ? options.searchPlaceholder : 'Busca'
					})
